...
```bash
# Install (core only - stdlib, no required third-party packages)
pip install -e .

# Install with optional QR code support (recommended)
pip install -e ".[qr]"

# Share a file - the link self-destructs after one download
onedrop secrets.json

# Recipient downloads (token is embedded in the URL printed at startup)
curl -O https://192.168.1.42/t/<token>/secrets.json
```
...
# Development install
make dev
# Equivalent to: pip install -e ".[dev,qr]"
```

### 2. TLS certificate

OneDrop requires TLS. You have three options, in order of preference:

**Option A - Use mkcert (recommended for local dev; gets you a trusted cert)**

```bash
# Install mkcert: https://github.com/FiloSottile/mkcert
make cert
# Generates onedrop.pem + onedrop-key.pem in the project root
```

**Option B - Use openssl for a self-signed cert**

```bash
openssl req -x509 -newkey rsa:4096 \
  -keyout onedrop-key.pem -out onedrop.pem \
  -days 365 -nodes -subj "/CN=onedrop" \
  -addext "subjectAltName=IP:192.168.1.42"
```

**Option C - Let OneDrop auto-generate one at startup**

If `cert.pem` / `key.pem` (or `onedrop.pem` / `onedrop-key.pem`) don't exist and you don't set `ONEDROP_CERT` / `ONEDROP_KEY`, OneDrop will generate a temporary self-signed cert via `openssl` and delete it on exit. No setup needed, but browsers will show a warning.
...
You can embed OneDrop in your own scripts or CI pipelines:

```python
import onedrop

# Generate persistent cert files (onedrop.pem + onedrop-key.pem)
# Uses mkcert if available; falls back to openssl
onedrop.setup()

# Share a file - blocks until the download limit is reached or Ctrl-C
onedrop.share(
    "path/to/secrets.json",
    max_downloads=1,      # default
    port=443,             # default
    show_qr=True,         # default
)
```

`onedrop.share()` accepts the same parameters as the CLI flags.
...
```bash
make dev      # install package + dev deps (pytest, ruff, mypy)
make check    # ruff + mypy + pytest - same gates as CI
make test     # pytest only
make lint     # ruff check only
make format   # ruff format (auto-fix)
make cert     # generate onedrop.pem + onedrop-key.pem via mkcert or openssl
```
